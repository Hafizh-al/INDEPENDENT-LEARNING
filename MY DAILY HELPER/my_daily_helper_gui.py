# my_daily_helper_gui.py
# Aplikasi GUI "My Daily Helper" - single-file
# Butuh: Python 3.x (bawaan Tkinter), tidak perlu library tambahan.
...
# -------------------------
# DATA SEDERHANA SEBAGAI "PENYIMPANAN"
# -------------------------
kontak = {}
transaksi = []
menu_makanan = {
    ...
    "Puding Telor": {"harga": 1500, "stok": 100},
}
nilai = {}
faq = {
    "harga": "Harga mulai dari Rp50.000.",
    "buka": "Kami buka setiap hari pukul 08:00 - 20:00.",
    "lokasi": "Kami ada di Purwokerto.",
}

# -------------------------
# FUNGSI UTIL (bantuan kecil)
# -------------------------
...
# -------------------------
# MAIN APP CLASS - bungkus semua GUI
# -------------------------
class MyDailyHelper(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("My Daily Helper")
        center_window(self, 900, 650)
        self.create_widgets()

    def create_widgets(self):
        # Tab control - setiap fitur di tab berbeda
        ...
        # Init builder per tab
        self.build_kontak_tab()
        self.build_keuangan_tab()
        self.build_analisis_tab()
        self.build_menu_tab()
        self.build_reminder_tab()
        self.build_chat_tab()
        self.build_pass_tab()
        self.build_nilai_tab()
    ...
    def add_contact(self):
        nama = self.k_nama.get().strip()
        nomor = self.k_nomor.get().strip()
        if not nama or not nomor:
            show_error("Nama dan nomor harus diisi")
            return
        kontak[nama] = nomor
    ...
    def delete_contact(self):
        ...
        if not sel:
            show_error("Pilih kontak di list untuk dihapus")
            return
    ...
    def search_contact(self):
        ...
        if not hasil:
            show_info("Tidak ditemukan")
            return
        # tampilkan hasil sementara
        self.lst_kontak.delete(0, 'end')
        for k, v in hasil.items():
            self.lst_kontak.insert('end', f"{k} -> {v}")

    def refresh_contacts_list(self):
        self.lst_kontak.delete(0, 'end')
        for k, v in sorted(kontak.items()):
            self.lst_kontak.insert('end', f"{k} -> {v}")
    ...
    def build_keuangan_tab(self):
        ...
        # daftar & total
        self.txt_trans = tk.Text(bottom, height=12)
        self.txt_trans.pack(expand=1, fill="both")
    ...
    def add_transaction(self):
        ...
        try:
            nom = float(self.t_nom.get().strip())
        except:
            show_error("Nominal harus angka (pakai titik untuk desimal).")
            return
    ...
    def refresh_transactions_view(self):
        ...
        for d, n in transaksi:
            self.txt_trans.insert('end', f"{d}: {n}\n")

    def show_total(self):
        total = sum(n for _, n in transaksi)
        show_info(f"Total saldo: {total}")
    ...
    def build_analisis_tab(self):
        ...
        self.txt_ana = tk.Text(frm, height=20)
        self.txt_ana.pack(expand=1, fill="both", padx=10, pady=8)
    ...
    def pick_text_file(self):
        ...
        try:
            ...
            show_error("Gagal baca file: " + str(e))
            return
        self.ana_file_label.config(text=path.split("/")[-1])
        # bersihkan dan hitung frekuensi
        ...
        self.txt_ana.insert('end', "Top kata:\n")
        for w, c in items:
            self.txt_ana.insert('end', f"{w} -> {c}\n")
    ...
    def build_menu_tab(self):
        ...
        # left: tampil menu & stok
        ttk.Label(left, text="Menu & Stok:").pack(anchor="w")
        self.lst_menu = tk.Listbox(left, height=10)
        self.lst_menu.pack()
        ...
        # right: kasir - tambah ke keranjang dan bayar
        ttk.Label(right, text="Kasir - pilih produk lalu 'Tambah'").pack(anchor="w")
        self.combo_produk = ttk.Combobox(right, values=list(menu_makanan.keys()))
        self.combo_produk.pack(pady=6)
        ttk.Button(right, text="Tambah ke Keranjang", command=self.add_to_cart).pack()
        self.lst_cart = tk.Listbox(right, height=8)
        self.lst_cart.pack(expand=1, fill="both", pady=6)
    ...
    def refresh_menu_list(self):
        ...
        for m, info in menu_makanan.items():
            self.lst_menu.insert('end', f"{m} - Rp{info['harga']} (Stok: {info['stok']})")
        self.combo_produk['values'] = list(menu_makanan.keys())

    def add_stock_prompt(self):
        sel = self.lst_menu.curselection()
        if not sel:
            show_error("Pilih item pada daftar menu terlebih dahulu")
            return
        line = self.lst_menu.get(sel[0])
        nama = line.split(" - ")[0]
        try:
            tambah = int(simpledialog.askstring("Tambah Stok", f"Berapa stok tambahan untuk {nama}?"))
        except:
            show_error("Masukkan angka valid")
            return
        menu_makanan[nama]['stok'] += tambah
        self.refresh_menu_list()
        show_info("Stok diperbarui")

    def add_to_cart(self):
        nama = self.combo_produk.get()
        if not nama or nama not in menu_makanan:
            show_error("Pilih produk yang tersedia")
            return
        if menu_makanan[nama]['stok'] <= 0:
            show_error("Stok habis!")
            return
        # kurangi stok & tambahkan harga ke keranjang
    ...
    def checkout(self):
        if not self.cart:
            ...
            return
        total = sum(self.cart)
        show_info(f"Total bayar: Rp{total}")
        self.cart.clear()
    ...
    def add_reminder(self):
        nama = self.r_name.get().strip()
        d = self.r_dead.get().strip()
        if not nama or not d:
            show_error("Isi nama tugas dan deadline")
            return
        try:
            dt = datetime.strptime(d, "%Y-%m-%d")
        except:
            show_error("Format tanggal harus YYYY-MM-DD")
            return
    ...
    def build_chat_tab(self):
        ...
        self.chat_box = tk.Text(frm, height=20)
        self.chat_box.pack(expand=1, fill="both", padx=10, pady=8)
    ...
    def chat_send(self):
        ...
        key = txt.lower()
        resp = faq.get(key, "Maaf, saya belum tahu jawabannya.")
    ...
    def build_pass_tab(self):
        ...
        self.spin_len = tk.Spinbox(frm, from_=6, to=64, width=6)
        self.spin_len.pack()
        ttk.Button(frm, text="Buat Password", command=self.make_password).pack(pady=6)
        self.pass_box = ttk.Entry(frm, width=50)
        self.pass_box.pack(pady=6)

    def make_password(self):
        try:
            ln = int(self.spin_len.get())
        except:
            show_error("Masukkan panjang valid")
            return
    ...
    def add_score(self):
        ...
        try:
            skor = float(self.n_score.get())
        except:
            show_error("Nilai harus angka")
            return
        if not nama:
            show_error("Isi nama")
            return
    ...
    def do_encrypt(self):
        ...
        try:
            ges = int(self.e_shift.get().strip())
        except:
            show_error("Masukkan angka untuk geser")
            return
        out = ""
        for ch in pes:
            if ch.isalpha():
                base = ord('A') if ch.isupper() else ord('a')
                # jaga di range alfabet (wrap around)
